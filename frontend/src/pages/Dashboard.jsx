...
export default function Dashboard() {
  ...
  // ──────────────────────────────────────────────
  // Transform backend response → frontend routes
  // ──────────────────────────────────────────────
  function transformRoutes(backendData) {
    const transformed = {};
    Object.keys(backendData).forEach(key => {
      const route = backendData[key];
      const config = ROUTE_CONFIG[key];
      if (!config) return;
      transformed[config.id] = {
        ...route,
        id: config.id,
        label: config.label,
  ...
  const handlePlanRoute = useCallback(async () => {
    ...
    try {
      const response = await axios.post(BACKEND_URL, { source, destination, stops, vehicleId, weather: 'clear' });
